...
- 研究使用者：按公开视图、关键词、组合标签、标签排除和年份范围筛选档案，并读取结构化导出。
...
5. **筛选导出**：`GET /collections/export` 在同一视图内筛选、排序并生成带校验和的集合；`GET /artifacts` 与导出共享筛选参数，支持多标签 all/any 组合、标签排除和闭区间年份范围，空结果返回空集合，非法组合返回参数错误。
